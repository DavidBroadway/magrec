...
class Ms2Bxyz(GenericTranformation):
    ...
    def __init__(self, dataset,  m_theta = 0, m_phi = 0):
        ...
        self.m_to_b = torch.einsum("...ijkl,i->...jkl", self.m_to_b_matrix, self.mag_dir)

        # sum over the sensor direction
        self.m_to_bx = torch.einsum("...jkl,j->...kl", self.m_to_b, self.sensor_x)
        self.m_to_by = torch.einsum("...jkl,j->...kl", self.m_to_b, self.sensor_y)
        self.m_to_bz = torch.einsum("...jkl,j->...kl", self.m_to_b, self.sensor_z)

        # Define the finally transformation
        self.trans_m_2_bx =  self.clean_transformation(self.m_to_bx)
        self.trans_m_2_by =  self.clean_transformation(self.m_to_by)
        self.trans_m_2_bz =  self.clean_transformation(self.m_to_bz)
    ...
    def transform(self, M = None):
        ...
        if len(M.size())<4:
            ...
            bx = m[0,:,:] * self.trans_m_2_bx
            by = m[0,:,:] * self.trans_m_2_by 
            bz = m[0,:,:] * self.trans_m_2_bz 

        else:
            # Transform the magnetization to the magnetic field
            bx = m[0,0,:,:] * self.trans_m_2_bx 
            by = m[0,0,:,:] * self.trans_m_2_by
            bz = m[0,0,:,:] * self.trans_m_2_bz
