...
class Mdir2Bxyz(GenericTranformation):
    ...
    def __init__(self, dataset):
        ...
        self.mag_dir_z = self.get_cartesian_dir(0, 0)
        ...
        self.mz_to_b = torch.einsum("...ijkl,i->...jkl", self.m_to_b_matrix, self.mag_dir_z)
        ...
        self.mz_to_bx = torch.einsum("...jkl,j->...kl", self.mz_to_b, self.sensor_x)
        ...
        self.mz_to_by = torch.einsum("...jkl,j->...kl", self.mz_to_b, self.sensor_y)
        ...
        self.mz_to_bz = torch.einsum("...jkl,j->...kl", self.mz_to_b, self.sensor_z)
    ...
    def transform(self, M = None, magnitude = 1):
        ...
        # Transform the spherical M into cartesian M 
        if len(M.size())>3:
            theta = M[0,0,::]
            phi = M[0,1,::]
        else:
        ...
        # Make a tensor of the magnetization direction with shape (1, 3, n_x, n_y)
        Mxyz = magnitude*torch.stack([ \
        ...
        m = self.ft.forward(Mxyz, dim=(-2, -1))

        # if len(M.size())<4:
            # Transform the magnetization to the magnetic field
        bx = m[0,:,:] * self.trans_mx_2_bx + m[1,:,:]*self.trans_my_2_bx + m[2,:,:]*self.trans_mz_2_bz
        by = m[0,:,:] * self.trans_mx_2_by + m[1,:,:]*self.trans_my_2_by + m[2,:,:]*self.trans_mz_2_by
        bz = m[0,:,:] * self.trans_mx_2_bz + m[1,:,:]*self.trans_my_2_bz + m[2,:,:]*self.trans_mz_2_bz

        # else:
        #     # Transform the magnetization to the magnetic field
        #     bx = m[0,0,:,:] * self.trans_mx_2_bx + m[0,1,:,:]*self.trans_my_2_bx + m[0,2,:,:]*self.trans_mz_2_bx
        #     by = m[0,0,:,:] * self.trans_mx_2_by + m[0,1,:,:]*self.trans_my_2_by + m[0,2,:,:]*self.trans_mz_2_by
        #     bz = m[0,0,:,:] * self.trans_mx_2_bz + m[0,1,:,:]*self.trans_my_2_bz + m[0,2,:,:]*self.trans_mz_2_bz
