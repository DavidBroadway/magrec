...
class Mxyz(GenericModel):
    ...
    def transform(self, nn_output):
        ...
        if self.source_weight is not None:
            if len(self.source_weight.shape) > 2:
                ...
                nn_output[0,2,::] = nn_output[0,2,::]*self.source_weight[2,::]
            else:
    ...
    def extract_results(self, final_output, final_b, remove_padding = True):
        ...
        """
        self.results = dict()
        self.results["Mx"] = final_output[0,0,::] / self.scaling_factor
        self.results["My"] = final_output[0,1,::] / self.scaling_factor
