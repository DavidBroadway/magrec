...
class Mab(GenericModel):
    ...
    def extract_results(self, final_output, final_b, remove_padding = True):
        ...
        """
        self.results = dict()
        self.results["Ma"] = final_output[0,0,::] / self.scaling_factor
        self.results["Mb"] = final_output[0,1,::] / self.scaling_factor
    ...
    def plot_results(self, results, brange = None, srange = None):  
        ...
        plt.subplot(4,3, 10)
        plot_data = results["Ma"]
        plot_range = abs(plot_data).max()
        plt.imshow(plot_data, cmap="PuOr", vmin=-plot_range, vmax=plot_range)
        plt.xticks([])
        plt.yticks([])
        cb = plt.colorbar()
        plt.title('reconstructed Ma')
        cb.set_label("M ($\mu_b/nm^2$)")

        plt.subplot(4,3,11)
        plot_data = results["Mb"]
        plot_range = abs(plot_data).max()
        plt.imshow(plot_data, cmap="PuOr", vmin=-plot_range, vmax=plot_range)
        plt.xticks([])
        plt.yticks([])
        cb = plt.colorbar()
        plt.title('reconstructed Mb')
        cb.set_label("M ($\mu_b/nm^2$)")
