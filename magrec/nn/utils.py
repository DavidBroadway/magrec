...
def plot_ffs_params(model, ax=None):
    """
    Plot Fourier features parameters of a model.
    
    Parameters:
    model : object
        The model containing Fourier features. It can be either a single 
        FourierFeaturesTransform or a model with multiple FourierFeaturesTransforms 
        collected under the `.ffs` attribute.
    ax : matplotlib.axes.Axes, optional
        The axes on which to plot. If None, a new figure and axes will be created.
    
    Returns:
    ax : matplotlib.axes.Axes
        The axes with the plotted Fourier features parameters.
    """
    ...
    if not hasattr(model, 'ffs'):
        # Assume then it's a single FourierFeaturesTransform
        ffs = [model] 
    else:
        # Assume it's a model with multiple FourierFeaturesTransforms
        ffs = model.ffs
    
    # Iterate through ffs (Fourier Feature layers) and plot the weights
    for i, ff in enumerate(ffs):
        Bx = ff.B[0, :].numpy()
        By = ff.B[1, :].numpy()
        # make markers as circles, not disks, for better visibility
        ...
        try:
            if hasattr(ff, 'std'):
                ...
                raise AttributeError("No valid attribute found in ff")
        ...
        ax.plot(Bx, By, 
                ...
                label=s_label)
        ax.set_aspect('equal')
        # make axis extents equal to the largest extent * 1.2
        ext = max(np.max(np.abs(Bx)), np.max(np.abs(By))) * 1.2
