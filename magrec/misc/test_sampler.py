...
def test_sampler():
    ...
    sampler_no_regen = Sampler(
        simple_sample_fn, batch_n_points, cached_n_points, regenerate_cache=False
    )

    batch_list_no_regen = [sampler_no_regen.sample_points(device) for _ in range(3)]

    assert (
        len(batch_list_no_regen) == 3
    ), "Failed to produce the correct number of batches"
    ...
    sampler_regen = Sampler(
        simple_sample_fn, batch_n_points, cached_n_points, regenerate_cache=True
    )

    batch_list_regen = [sampler_regen.sample_points(device) for _ in range(4)]
    ...
    assert (
        len(batch_list_regen[3]) == 7
    ), "Fourth batch size (after regeneration) is incorrect"
...
def test_sampler_visual():
    ...
    for i in range(num_batches):
        ...
        axs[0].scatter(
            torch.ones(len(batch)) * (i % num_batches_to_exhaust_cache)
            + 0.1 * (i // num_batches_to_exhaust_cache),
            batch,
            color=colors[i],
        )
...
def test_grid_sampler():
    ...
    ax.scatter(
        grid_points[:, 0], grid_points[:, 1], marker="o", c=colors, cmap="viridis", s=10
    )  # Adjusted marker and size for clarity
    ...
    cbar = plt.colorbar(
        ax.scatter(
            grid_points[:, 0], grid_points[:, 1], c=colors, cmap="viridis", s=10
        ),
        ax=ax,
...
def test_grid_sampler_pts_to_grid():
    ...
    for i in range(n_x):
        for j in range(n_y):
            x, y = grid[:, i, j]
            expected_x, expected_y = pts[j * n_x + i]
            assert (
                x == expected_x and y == expected_y
            ), f"Expected ({expected_x}, {expected_y}), but got ({x}, {y}) at ({i}, {j})"


@auto_reference_plot
def test_grid_sampler_gridder_ungridder():
    """
    Test whether pts provided by a grid sampler end up in a regular grid as expected. 
    """
    from magrec.misc.sampler import GridSampler
    ...
    y = pts[:, 1]
    f = torch.sin(y) * x ** 2 
    f = GridSampler.pts_to_grid(f, nx, ny)
    fig = plot_n_components(f, show=True)
    return fig
